...
import { generateAccessToken, generateRefreshToken } from '../../infraestructure/utils/tokenGenerator';
...
export class TokenService {
  ...
  async exchangeRefreshToken(params: { refreshToken: string; clientId: string; clientSecret?: string }): Promise<Token> {
    ...
    const updatedToken = new Token(accessToken, newRefreshToken, token.clientId, token.scopes, token.expiresIn);
    await this.tokenRepository.save(updatedToken);

    return updatedToken;
  }

  async clientCredentials(params: { clientId: string; clientSecret: string }): Promise<Token> {
    ...
    const token = new Token(accessToken, refreshToken, clientId, client.scopes, 3600);
    await this.tokenRepository.save(token);
